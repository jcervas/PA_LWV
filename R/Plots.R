...
district_per <- abs(50 - round(district_votes.2016 * 100, digits = 0))

# Gradiant Version
pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_barplotPartisanship.pdf"), width=8, height=8)
  par(mfrow=c(1,9),
  	mar=c(0,0,0,0))
  for (i in 1:8) 
    {
...
# % -----------------------------------------------------------------
yrs <- seq(2012,2016,2)
pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_sims_actual_density.pdf"), 
	width = 5, 
	height = 6)
par(mfrow=c(3,1), 
	oma= c(3,1.5,0,0), 
...
for (k in 1:3)
	{		
		x <- do.call(rbind, comp.sims.seats.years[k])
		x.sd <- sd(x)
		x.mean <- mean(x)
		a <- density(x, na.rm=TRUE, from=x.mean-x.sd, to=x.mean+x.sd)
		area <- abs(sum(a$y)*(a$x[1]-a$x[2]))
...
pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_sims_2018_maps_density.pdf"), 
	width = 8, 
	height = 10)
par(mfrow=c(4,2), 
	oma= c(3,1.5,0,0), 
...
# MAP SIMULATIONS OF PLANS - 2016
# % =================================================================
# % -- FIGURE -- FIGURE -- FIGURE -- FIGURE -- FIGURE -- FIGURE --  %
# % -----------------------------------------------------------------

pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_sims_2016_maps_density.pdf"), 
	width = 8, 
	height = 10)
par(mfrow=c(4,2), 
	oma= c(3,1.5,0,0), 
...
pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_sims_5050_maps_density.pdf"), 
	width = 8, 
	height = 10)
par(mfrow=c(4,2), 
	oma= c(3,1.5,0,0), 
...
# % -- FIGURE -- FIGURE -- FIGURE -- FIGURE -- FIGURE -- FIGURE --  %
# % -----------------------------------------------------------------
pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_compactness.pdf"), width = 6, height = 6)
	par(
		mfrow = c(2, 2), 
		mar = c(0, 0, 0, 0), 
		oma = c(0, 0, 0, 0), 
...
# % =================================================================
  ...
  # Plot Plan Maps
		...
		c <- rbind(rim,c, c[1,])
		dev.off()
pdf(paste0("Papers/(2019) Tools for Identifying Partisan Gerrymandering (PA)/Figures/fig_maps.pdf"), 
  	width=8, 
  	height=10)
par(mar=c(0,0,0,0),
	bg="white")
